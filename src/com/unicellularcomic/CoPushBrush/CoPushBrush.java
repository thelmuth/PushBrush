...
import com.unicellularcomic.PushBrush.*;

public class CoPushBrush extends PApplet {
	...
	public void setup() {
		...
		// Setup PushBrushPC
		try {
			String[] parameterStrings = loadStrings("copushbrush.pushgp");
			...
			if (!(gaInitial instanceof CoPushBrushPC)) {
				throw new Exception(
						"ERROR: The problem-class must inherit from PushBrushPC.");
			}

			ga = (CoPushBrushPC) gaInitial;

			// Just to prime painting individual
			//ga.RunUntilFitnessPrediction();
	 ...
	 * 
	 * @param brushToUpdate
	 *            - Brush to update with newBrush attributes
	 * @param newBrush
	 *            - New brush attributes
	 */
	private void updateBrush(BrushAttributes brushToUpdate,
			BrushAttributes newBrush) {
	 ...
	 * 
	 * @return true if draw() needs to return false if draw() can continue
	 */
		...
		if (bestBrushPaintButton.clicked()) {
			...
			if (MOUSE_INTERACTION_ENABLED) {
				freeDrawBrush = new BrushAttributes(initx, inity, initradius,
						initr, initg, initb, initalpha, inittimeStep, true);
			} else {
				freeDrawBrush = new BrushAttributes(initx, inity, initradius,
	...
	/**
	 * Runs a step of the GA during each draw() cycle. humanEvaluated is true if
	 * a human evaluation was made.
	 */
	private void runGAStep(boolean humanEvaluated) {
		...
		if (humanEvaluated) {
			...
			ga.EvaluateTrainerAndGetNext(fitness);
			...
			imgCanvas = createImage(canvasWidth, canvasHeight, RGB);
			if (MOUSE_INTERACTION_ENABLED) {
				brush = new BrushAttributes(initx, inity, initradius, initr,
						initg, initb, initalpha, inittimeStep, true);
			} else {
				brush = new BrushAttributes(initx, inity, initradius, initr,
						initg, initb, initalpha, inittimeStep);
			}

		}

		if (firstTrainerEvaluated) {
			// Evaluate next individual using prediction
			try {
				ga.RunUntilFitnessPrediction();
			} catch (Exception e) {
	...
	private void drawTextScreen() {
		...
		if (codeViewBrushButton.clicked()) {
			...
			if (!programIsLegal) {
				...
				background(0);
				if (MOUSE_INTERACTION_ENABLED) {
					freeDrawBrush = new BrushAttributes(initx, inity,
